...
const scheduledDates = computed(() => {
  if (kitchenStore.activeMenuPart) {
    ...
    for (let i = 0; i < intermediate; i++) {
      const date = new Date(
        kitchenData.value.startDate.split(".").reverse().join("-")
      );
      date.setDate(date.getDate() + i);
      const formattedDate = formatDate(date);
      formattedDates.push({
        date: formattedDate.date,
        title: `${formattedDate.week} - ${formattedDate.date}`,
      });
    }
    ...
    const intermediate =
      getDaysInterval(kitchenData.value.startDate, kitchenData.value.endDate) +
      1;
    const formattedDates = [];
    for (let i = 0; i < intermediate; i++) {
      const date = new Date(
...
const copyDataToAnotherDay = (sourceDate: string, targetDate: string) => {
  const sourceListDishes = list_dishes.value.find(d => d.date === sourceDate);
  const sourceData = data.value.find(d => d.date === sourceDate);

  if (!sourceListDishes && !sourceData) {
    return;
  }

  if (sourceListDishes) {
    const copiedData = JSON.parse(JSON.stringify(sourceListDishes.data));

    const targetIndex = list_dishes.value.findIndex(d => d.date === targetDate);

    if (targetIndex !== -1) {
      list_dishes.value[targetIndex].data = copiedData;
    } else {
  ...
  if (sourceData) {
    const copiedData = JSON.parse(JSON.stringify(sourceData.data));

    const targetIndex = data.value.findIndex(d => d.date === targetDate);
    if (targetIndex !== -1) {
      data.value[targetIndex].data = copiedData;
    } else {
      data.value.push({
        date: targetDate,
        data: copiedData,
...
const copyToNextDay = () => {
  const activeIndex = scheduledDates.value.findIndex(
    d => d.date === activeScheduledDate.value
  );
  if (activeIndex === -1 || activeIndex >= scheduledDates.value.length - 1) {
    return;
  }

  const nextDate = scheduledDates.value[activeIndex + 1].date;

  copyDataToAnotherDay(activeScheduledDate.value, nextDate);

  activeScheduledDate.value = nextDate;
};
...
const canCopyData = computed(() => {
  ...
  const hasCheckedMealData = currentData?.data.some(
    meal => meal.isChecked && meal.mealData.length > 0
  );

  const currentListDishes = list_dishes.value.find(
    d => d.date === activeScheduledDate.value
  );
  const hasCheckedListDishesData = currentListDishes?.data.some(
    meal => meal.isChecked && meal.mealData.length > 0
  );

  return hasCheckedMealData || hasCheckedListDishesData;
});
  ...
  if (route.name === "KitchenMenuEdit") {
    ...
    if (kitchenStore.activeMenuPart) {
      kitchenData.value.startDate = kitchenStore.menuItem.menu.start_date
        ...
        .reverse()
        .join("-");
      kitchenData.value.intermediateDate1 =
        kitchenStore.menuItem.menu.duration == 7;
    }
    if (kitchenStore.activeSalesPart) {
      kitchenData.value.startDate = kitchenStore.menuItem.menu.start_date
        .split(".")
...
const scheduledItemDateClass = (date: string): string[] => {
  ...
  if (route.name === "KitchenMenuCreate") {
    ...
    classes.push(
      "text-dark-gray transition duration-200 ease-in text-xs py-2 px-3"
    );
...
const changeRation = async (
  ...
  if (val) {
    ...
    // console.log(data);
    data.value[parentIndex].data[childIndex].mealData[indexMeal].rationsList =
      ...
      for (let meal of datum.mealData) {
        if (kitchenStore.activeSalesPart) {
          ...
          if (!meal["meal"] && !meal["vid_product"]) {
            commonStore.errorToast(
              t("form.validate.emptyField") + ": meal or vid_product"
            );
            return false;
          }
        } else {
          ...
          for (let key of Object.keys(meal)) {
            if (!meal[key]) {
              commonStore.errorToast(
                t("form.validate.emptyField") + `: ${key}`
              );
              return false;
            }
          }
...
const sendData = async () => {
  if (kitchenStore.activeMenuPart) {
    ...
    if (route.name !== "KitchenMenuEdit") {
      ...
      await kitchenStore.UPDATE_MENU_ELEMENT(menu_id, {
        Elements: kitchenElementPayload,
      });
    }
  }
      ...
      data
        ...
        .flatMap(meal =>
          meal.mealData.map(mealDetail => ({
            ...
            date: date,
            ["product_id"]: mealDetail.meal
              ? mealDetail.meal
              : mealDetail.vid_product,
  ...
  await router.push(
    `/kitchen/${route.params.department_id}/${
      kitchenStore.activeSalesPart ? "sales" : "free-kitchen"
    }/${route.params.kitchen_id}/${route.params.child_id}/menu`
  );
};
...
watch(
  ...
  async () => {
    await kitchenStore.GET_KITCHEN_VID({
      ...
      is_paid:
        route.params.part_name == "free-kitchen"
          ? 0
          : route.params.part_name == "sales"
          ? 1
          : null,
    });
...
watch(
  () => kitchenData.value.intermediateDate1,
  async val => {
    await settingsStore.GET_RATION_LIST({
      per_page: 100,
      duration: val ? 7 : 10,
      kitchen_id: route.params.child_id,
    });
...
const changeInput = async (
  ...
  if (typeof event !== "object") {
    ...
    list_dishes.value[index].data[childIndex].mealData[indexMeal].vid_list =
      data.product_types ? data.product_types : [];
  }
};

const changeInputMeals = async (
  event: any,
  index: number,
  childIndex: number,
  indexMeal: number
  ...
  if (typeof event !== "object") {
    ...
    list_dishes.value[index].data[childIndex].mealData[indexMeal].meals_list =
      meal ? meal : {};
  }
...
const changeChange = async (
  event: any,
  index: number,
  childIndex: number,
  indexMeal: number
) => {
  list_dishes.value[index].data[childIndex].mealData[indexMeal].vid_product =
    null;
...
const deleteMeal = (
  ...
  list_dishes.value[index].data[childIndex].mealData.splice(indexMeal, 1);
};
          ...
          <div class="mt-6">
            ...
            >
              <div v-for="(item, index) in data">
                ...
                >
                  <div v-for="(childItem, childIndex) in item.data">
                    <ElCheckbox
                    ...
                    >
                      ...
                      <div v-for="(itemMeal, indexMeal) in childItem.mealData">
                        ...
                        <div
                          v-if="
                            itemMeal.product_id &&
                            itemMeal.rationsList &&
                            itemMeal.rationsList.products
                          "
                          class="mt-6"
                        >
                          <ElTable
                          ...
                          >
                            ...
                            <template #append>
                              <div
                                class="px-4 py-3.5 flex justify-end items-center gap-x-8"
                              >
                                <div class="flex items-center gap-x-1 text-sm">
                                  ...
                                  <strong class="font-semibold text-dark">
                                    {{
                                      itemMeal.rationsList?.price.toLocaleString()
                                    }}
                                    {{ t("currency.sum") }}
                                  </strong>
                                </div>
                                <div class="flex items-center gap-x-1 text-sm">
                                  ...
                                  <strong class="font-semibold text-dark">
                                    {{
                                      itemMeal.rationsList?.vat_price.toLocaleString()
                                    }}
                                    {{ t("currency.sum") }}
                                  </strong>
                                </div>
            ...
            <div
              v-else-if="kitchenStore.activeSalesPart && list_dishes.length > 0"
            >
              <!--              {{ list_dishes }}-->
              <!--              {{ activeScheduledDate }}-->
                      ...
                      <div v-for="(item, indexMeal) in childItem.mealData">
                        <div class="flex items-center gap-4 w-full">
                          ...
                          <app-select
                            ...
                            itemLabel="name"
                            :items="
                              settingsStore.typeProduct.product_categories
                            "
                            @input="
                              changeInput($event, index, childIndex, indexMeal)
                            "
                            @change="
                              changeChange($event, index, childIndex, indexMeal)
                            "
                          />
                          <app-select
                          ...
                          <button
                            ...
                            @click="deleteMeal(index, childIndex, indexMeal)"
                          >
                            <img
                        ...
                        >
                          ...
                          >
                            ...
                            <template #append>
                              <div
                                class="px-4 py-3.5 flex justify-end items-center"
                              >
                                <div class="flex items-center gap-x-8">
                                  ...
                                  >
                                    ...
                                    <strong class="font-semibold text-dark">
                                      {{
                                        item.meals_list.net_price &&
                                        item.meals_list.net_price.toLocaleString()
                                      }}
                                      {{ t("currency.sum") }}
                                    </strong>
                                  </div>
                                  ...
                                  >
                                    ...
                                    <strong class="font-semibold text-dark">
                                      {{
                                        item.meals_list.tax &&
                                        item.meals_list.tax.toLocaleString()
                                      }}
                                      {{ t("currency.sum") }}
                                    </strong>
                                  </div>
                                  ...
                                  >
                                    ...
                                    <strong class="font-semibold text-dark">
                                      {{
                                        item.meals_list.price &&
                                        item.meals_list.price.toLocaleString()
                                      }}
                                      {{ t("currency.sum") }}
                                    </strong>
                                  </div>
