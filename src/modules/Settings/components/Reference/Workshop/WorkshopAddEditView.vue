...
const workshopData = ref<WorkshopType>({
  name: {
    ru: "",
    uz: "",
  },
...
onMounted(async () => {
  ...
  if (route.params.id) {
    const data = await settingsStore.GET_WORKSHOP_ITEM(route.params.id as string | number);

    if (data && data.workshop) {
      workshopData.value = data.workshop;
      status.value = data.workshop.status == "active";
    }
  }
});
...
const cancelFn = () => {
  confirm.cancel().then(() => {
    router.push({ name: "reference-workshop" });
  });
};

const deleteFn = () => {
  confirm.delete().then(async res => {
    await settingsStore.DELETE_WORKSHOP(route.params.id as string);
    await router.push({ name: "reference-workshop" });
  });
};
...
const handleSubmit = async () => {
  ...
  if ((await v$.value.validate())) {
    ...
    try {
      ...
      if (route.params.id) {
        await settingsStore.UPDATE_WORKSHOP({
          id: route.params.id as string | number,
          data: workshopData.value,
        });
      } else {
        await settingsStore.CRETE_WORKSHOP(workshopData.value);
      }
      await commonStore.successToast("/reference-workshop");
    } catch (e) {
...
const switchChange2 = async (e: any): Promise<boolean> => {
  if (status.value) {
    workshopData.value.status = "active";
  } else {
    workshopData.value.status = "inactive";
  }
};

const isDisabled = computed<boolean>(() => {
  return route.name == "reference-workshop-view";
});
...
<template>
  <div>
    ...
    <div class="flex gap-6">
      <div class="w-[70%]">
        ...
        >
          ...
          >
            ...
            <div class="flex items-center gap-4">
              <AppSelect
                v-model="workshopData.factory_id"
      ...
      <div class="w-[30%]">
        <button
          @click="router.push({name: 'reference-workshop-edit', params: {id: 1}})"
