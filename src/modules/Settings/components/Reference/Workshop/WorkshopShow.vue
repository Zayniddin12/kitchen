...
onMounted(() => {
  setBreadCrumbFn();
  settingsStore.GET_WORKSHOP_ITEM(+route.params.id);
});
...
<template>
  <section>
    ...
    <div class="flex items-start mt-6">
      <AppOverlay
        :loading="settingsStore.workshopItemLoading"
        parent-class-name="app-card w-[70%] min-h-[65vh]"
        class="grid grid-cols-2 gap-6"
        :rounded="16"
      >
        <template v-if="settingsStore.workshopItem">
          <div>
            <span class="app-card__item-title">
              {{ t("Наименование цеха Рус:") }}
            </span>
            <p>
              {{ settingsStore.workshopItem.name[activeLocale] }}
            </p>
          </div>
          <div>
            <span class="app-card__item-title">
          ...
          <div>
            ...
            <p>
              {{ settingsStore.workshopItem.capacity }}
            </p>
          </div>
          <div>
            <span class="app-card__item-title">
      ...
      <RouterLink
        ...
        :to="{name: 'reference-workshop-edit'}"
